...
require('github_immutable_subject_prefixes', wif, 'immutable default GitHub subject catalog')
require(
...
for token, label in (
    ...
    ('${local.github_immutable_subject_prefixes[local.github_signer_repository]}:environment:${local.github_signer_environment}', 'immutable protected release signer subject'),
...
require('repository_owner_id = var.github_org_id', module_outputs, 'exported immutable GitHub owner ID')
require(
...
require('"google.subject"               = "assertion.pipeline_id"', wif, 'bounded immutable Buildkite subject mapping')
buildkite_block = wif.split('resource "google_iam_workload_identity_pool_provider" "buildkite"', 1)[-1]
if '"google.subject"               = "assertion.sub"' in buildkite_block:
    errors.append('Buildkite provider maps the potentially overlong default compound subject')
...
if 'repo:${var.github_org}/${repo}' in wif or 'repo:${var.github_org}/${local.github_signer_repository}' in wif:
...
for name, workflow in (('drift', drift_workflow), ('recovery inspection', recovery_workflow)):
